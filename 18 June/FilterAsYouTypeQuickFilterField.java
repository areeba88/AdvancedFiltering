...
/*
 * @(#)FilterAsYouTypeQuickFilterField.java 6/6/2013
 *
...
public class FilterAsYouTypeQuickFilterField   extends AbstractDemo {
	...
	public FilterAsYouTypeQuickFilterField() {
	...
	public Component getDemoPanel() {
		...
		QuickTableFilterField   _filterField = new QuickTableFilterField(_groupTableModel);
		
	
		_table = new GroupTable(_filterField.getDisplayTableModel()){
...
//Col#1
		...
		addListenerWithTxtField(_filterField,filterFiledName,_groupTableModel);
//Col#2
		...
		addListenerWithTxtField(_filterField,filterFiledID,_groupTableModel);
//Col#3
		...
		addListenerWithTxtField(_filterField,filterFiledCGPA,_groupTableModel);
...
//Col#4
		...
		addListenerWithTxtField(_filterField,filterFiledDate,_groupTableModel);
	...
	class TextFiledCellEditor extends DefaultCellEditor {
		...
		public TextFiledCellEditor(/*QuickTableFilterField _filterField,*/JTextField textField) {
			super(textField);
	...
	private void addListenerWithTxtField(QuickTableFilterField _filterField,JTextField txt,TableModel _tableModel){
	
		txt.addKeyListener(new QucikFilteringKeyListener(_filterField,txt, _tableModel));
	}
	...
	class QucikFilteringKeyListener extends KeyAdapter {
		...
		public QucikFilteringKeyListener(QuickTableFilterField _filterField,JTextField txt,  TableModel _tableModel){
		...
		public void keyReleased(KeyEvent e) {
		 ...
		 if(txtString!=null ){
				if(txt.getName().equals("FirstName"))
				{
					...
					_filterField.setSearchingColumnIndices(new int[]{1});
					_filterField.setSearchingText(txt.getText());
										
				}
				if(txt.getName().equals("LastName"))
				{
					_filterField.setSearchingColumnIndices(new int[]{2});
					_filterField.setSearchingText(txt.getText());
				...
				if(txt.getName().equals("CGPA"))
				{
					...
					_filterField.setSearchingColumnIndices(new int[]{3});
					_filterField.setSearchingText(txt.getText());
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				panel.add(new FilterAsYouTypeQuickFilterField().getDemoPanel());
				...
				btn.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						int rowCount=FilterAsYouTypeQuickFilterField._groupTableModel.getRowCount();

						for (int i = 1; i < rowCount-1; i++) {

							FilterAsYouTypeQuickFilterField._groupTableModel.setValueAt(FilterAsYouTypeQuickFilterField.parseDate(dateData[i]), i, 4);
						}
