...
//					((DynamicTableFilter)super.getDynamicTableFilters(modelIndex)).
				}
					...
					return "DateFilterSP";
						...
						public boolean initializeFilter(TableModel tableModel, int i, Object[] objects) {
							...
							if(i==_groupTableModel.findColumn("DateTime")) {
								//if(){
								System.out.println("DateTime Index"+i);
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				JButton btn2 = new JButton("DateFilter");
				panel.add(btn2);
				...
				try{
					...
					DynamicTableFilter[] dynamicFilters = filterableTableModel.getDynamicTableFilters(8); 
				for(int i=0;i<dynamicFilters.length;i++){
					System.out.println(""+dynamicFilters.toString());
	...
	class DateFilterListener implements ActionListener
	{
		...
		private void dateTypeFilterApplied(Filter dateFilter){
			...
			JOptionPane.showMessageDialog(null, "sssssssss"); 
		}
